...
- `.harness/rules/install-rules.md`：定义 `install-harness` 固定资产释放、Harness core self-check、`project-init` 语义入口集成、`project-env-contract` 环境契约生成，以及 `session-start.py` / `check-project-env.py` 进入实际 workflow 的顺序与边界。
